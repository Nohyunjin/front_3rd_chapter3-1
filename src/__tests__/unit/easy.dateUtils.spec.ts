...
describe('getDaysInMonth', () => {
  ...
  it('유효하지 않은 월에 대해 적절히 처리한다', () => {
    const result13 = getDaysInMonth(2024, 13);
    const result0 = getDaysInMonth(2024, 0);
    const resultNegative = getDaysInMonth(2024, -1);

    expect(result13).toBe(31);
    expect(result0).toBe(31);
    expect(resultNegative).toBe(30);
  });
});
...
describe('getEventsForDay', () => {
  it('특정 날짜(1일)에 해당하는 이벤트만 정확히 반환한다', () => {
    const events = [{ id: '1', date: '2024-07-01' }];
...
describe('formatWeek', () => {
  ...
  it('연도가 바뀌는 주에 대해 올바른 주 정보를 반환한다', () => {
    ...
    const result = formatWeek(date);
    expect(result).toBe('2024년 12월 4주');
  });
